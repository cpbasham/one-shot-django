  ...
  if (target.is(".unviable")) {
    ...
    if (target.is($("ul.map-selector").children())) {
      ...
      $.ajax({
        ...
        method: "POST",
        data: "map=" + target.data("map")
      });
    } else {
      $.ajax({
        url: "/item_sets/" + currentSetId(),
        method: "POST",
        data: "game-type=" + target.data("game-type")
      });
    }
    bringToTop(target);
  }
...
function bringToTop(jListItem) {
  ...
  if (jListItem.data("map") !== undefined && jListItem.data("map") === first.data("map")) {  return; }
  if (jListItem.data("game-type") !== undefined && jListItem.data("gameType") === first.data("gameType")) { return; }
